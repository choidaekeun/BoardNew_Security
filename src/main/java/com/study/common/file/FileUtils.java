...
public class FileUtils {
    ...
    public List<FileRequest> uploadFiles(final List<MultipartFile> multipartFiles) {
        ...
        //MultipartFile 객체에 사용자가 업로드한 파일 정보가 담김
    ...
    public FileRequest uploadFile(final MultipartFile multipartFile) {
        ...
        String uploadPath = getUploadPath(today) + File.separator + saveName; //파일의 업로드 경로
        File uploadFile = new File(uploadPath); //업로드할 파일 객체

        try {
            multipartFile.transferTo(uploadFile); //파일 생성(write) 완료
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        
        //결과적으로 해당 메서드가 리턴하는 객체에는 디스크에 생성된 파일 정보가 담김
        //리턴 타입 FileRequest 객체
    ...
    /**
     * 파일 삭제 (from Disk)
     * @param addPath - 추가 경로(업로드 연월일 폴더를 처리하기 위해 사용되는 파라미터)
     * @param filename - 파일명
     */
    ...
    public Resource readFileAsResource(final FileResponse file) {
    	...
    	try {
            ...
            System.out.println(resource.exists()  + "/" + resource.isFile());
            if (resource.exists() == false || resource.isFile() == false) { // 리소스가 없거나, 리소스의 타입이 파일이 아닌 경우
